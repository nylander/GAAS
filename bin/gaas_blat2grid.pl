...
my $outdir = "blat_output";
my $db = undef;
my $fasta = undef;
my $chunk_size = 500;		# Partition size of fasta input
...
my $logfile = "$outdir/blat2grid.log";
...
for (my $i=1;$i<=$chunk_counter;$i++) {
	my $cmd = "blat -minIdentity=98 -minScore=80 $db $outdir/chunk_$i.fa $outdir/chunk_$i.psl";
	push(@cmds,$cmd);
}
...
foreach my $file (@files) {
	system("cat $file >> $outdir/blat.merged");
}

msg("Finished BLAT grid run.");
...
The name of the nucleotide/protein fasta file to use as query.

=item B<--db>

The name of the database use to blat
