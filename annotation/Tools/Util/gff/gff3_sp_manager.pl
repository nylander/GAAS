...
sub parse_interpro_tsv {
  ...
  while( my $line = <$file_in>)  {    
    ...
    if( $evalue < $opt_interproscanEvalue ) {
      
      #Cehck for the specific DB
      my $db_name=$values[3];
      my $db_value=$values[4];
      print "Specific dB: ".$db_name." ".$db_value."\n" if($opt_verbose);

      $TotalTerm{$db_name}++;
      push ( @{$functionData{$db_name}{$mRNAID}} , $db_value );
      ...
      if( $sizeList>11 ){
        ...
        $interpro_value=~ s/\n//g;
        print "interpro dB: ".$db_name." ".$db_value."\n" if($opt_verbose);

        $TotalTerm{$db_name}++;
        push ( @{$functionData{$db_name}{$mRNAID}} , $interpro_value );
        if ( exists $hash_mRNAGeneLink->{$mRNAID}){ ## check if exists among our current gff annotation file analyzed
          $mRNAAssociatedToTerm{$db_name}{$mRNAID}++;
          $GeneAssociatedToTerm{$db_name}{$hash_mRNAGeneLink->{$mRNAID}}++;
        }
      ...
      if( $sizeList>13 ){
        ...
        foreach my $go_tuple (@go_list){
          my @tuple = split(/:/,$go_tuple); #cut at character :
          my $go_value = $tuple[1];
          print "GO dB: ".$db_name." ".$go_value."\n" if($opt_verbose);

          $TotalTerm{$db_name}++;
          push ( @{$functionData{$db_name}{$mRNAID}} , $go_value );
          if ( exists $hash_mRNAGeneLink->{$mRNAID}){ ## check if exists among our current gff annotation file analyzed
            $mRNAAssociatedToTerm{$db_name}{$mRNAID}++;
            $GeneAssociatedToTerm{$db_name}{$hash_mRNAGeneLink->{$mRNAID}}++;
          }
      ...
      if( $sizeList>14 ){
        ...
        foreach my $pathway_tuple (@pathway_list){
          ...
          my $db_name = $tuple[0];
          my $pathway_value = $tuple[1];
          print "pathway dB: ".$db_name." ".$pathway_value."\n" if($opt_verbose); 

          $TotalTerm{$db_name}++;
          push ( @{$functionData{$db_name}{$mRNAID}} , $pathway_value );
          if ( exists $hash_mRNAGeneLink->{$mRNAID}){ ## check if exists among our current gff annotation file analyzed
            $mRNAAssociatedToTerm{$db_name}{$mRNAID}++;
            $GeneAssociatedToTerm{$db_name}{$hash_mRNAGeneLink->{$mRNAID}}++;
          }
