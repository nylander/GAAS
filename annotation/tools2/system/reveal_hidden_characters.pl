...
my $opt_infile;
my $opt_column="description";
my $opt_out;
my $opt_help = 0;


if ( !GetOptions( 'f|infile=s' => \$opt_infile,
                  'o|out|output=s' => \$opt_out,
                  'c|column=s'	=> \$opt_column,
                  'h|help!'         => \$opt_help ) )
{
...
if ($opt_column) {
	$opt_column = lc($opt_column);
	$opt_column =~ tr/ /_/;
	if( ! exists($column{$opt_column})){
		print "$opt_column does not exist. Choice must be among <DEC>, <OCT>, <HEX>, <BIN>, <Symbol_HTML>, <Number_HTML>, <Name> and <Description>. Whithout the <> characters (it is case insensitive).\n"; exit;
	}
}
...
if ( defined($opt_out) ) {
	if (-f $opt_out) {
 		print "$opt_out output directory already exits !\n";exit;
		}

    $outstream->open( $opt_out, 'w' ) or
      croak( sprintf( "Can not open '%s' for writing %s", $opt_out, $! ) );
}
else {
...
open FILE, $opt_infile or die "Couldn't open $opt_infile";
...
while (<FILE>) {
  ...
  foreach my $char (split //){
  	...
  	my $ascii_dec = ord($char);
  	my $desc = $ascii{$ascii_dec}[$column{$opt_column}];
  	printf "character:<%s>  | ascii decimal:%3d | %s:%s\n", $char,, $ascii_dec, $opt_column, $desc;
...
reveal_hidden_charracter.pl - 

It happens we have problem with some characters in a file e.g non-printable chracter.
This script decode character by character. For each character it will print:
the character itself | The ascii decimal | An extra information
The extra information can be choosen between  <DEC>, <OCT>, <HEX>, <BIN>, <Symbol_HTML>, <Number_HTML>, <Name> and <Description> (Decription by default)


ABOUT THE ASCII TABLE used (https://www.ascii-code.com):
================================
...
=head1 SYNOPSIS
    ...
    ./reveal_hidden_charracter.pl --f=infile [--output=file]
    ./reveal_hidden_charracter.pl --help

=head1 OPTIONS

=over 8

=item B<--infile> or B<-f>

Input file containing DNA.

=item  B<--desc> or B<-d>

Specify extra information/description you want for the character.  Choice must be among <DEC>, <OCT>, <HEX>, <BIN>, <Symbol_HTML>, <Number_HTML>, <Name> and <Description>. Whithout the <> characters. 
The option is case insensitive.
By default it is <Description>.

=item  B<--out>, B<--output> or B<-o>

[OPTIONAL] Output file. If no output is specified, the result is written to STDOUT.
